...
#-------------------------This section performs cuts based on what the user desires----------------------------------     
    ...
    if (TEST_MODE): #--------Stack of points for testing the codes-----------------------------------------------
        ...
        position_vector_cut2=numpy.array([numpy.array([0.01,i*TEST_LENGTH,0.]) for i in numpy.arange(1,10).astype('float')]) 
        position_vector_cut3=numpy.array([numpy.array([0.01,i*TEST_LENGTH,0.]) for i in numpy.arange(1,10).astype('float')]) 
        position_vector_cut4=numpy.array([numpy.array([3.,i*TEST_LENGTH,0.]) for i in numpy.arange(1,10).astype('float')]) 
...
TEST_LENGTH=0.1 #--------Testing linking length if ON. See construction of points for details
...
log_luminosity_cuts_space=[42,43,44,45]
...
for redshift in redshift_space:
    for log_luminosity_cuts in log_luminosity_cuts_space:
        for log_LINKING_LENGTH in log_LINKING_LENGTH_space:
            ...
            #FOF=0
            #final_FOF_tag1,subsampled_quantities1=generate_tags(log_luminosity_cuts,redshift,log_LINKING_LENGTH,'FIRST')

            FOF=1
            final_FOF_tag,subsampled_quantities=generate_tags(log_luminosity_cuts,redshift,log_LINKING_LENGTH,'FIRST')
            if (~TEST_MODE):
                pickle.dump([final_FOF_tag,subsampled_quantities],open('/nfs/nas-0-1/akbhowmi/FOF_tag_outputs/log_luminosity_cut_%.1f_redshift_%.2f_log_LINKING_LENGTH_%.2f.pickle'%(log_luminosity_cuts,redshift,log_LINKING_LENGTH),'w'))
